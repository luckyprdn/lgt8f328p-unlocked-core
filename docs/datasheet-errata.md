...
| DOC-018 | LGT8F328D/E datasheet ADC prose says "ADC inputs 0..5 can be differential; gain selectable via ADTMR GAIN", but the detailed ADTMR register map (authoritative) has only bit 0 (ADTM, test-mode output of internal reference on AVREF) — there is no GAIN field. | No ADTMR GAIN bits defined. The real differential front-end on 328D/E is OPA0/OPA1 whose output feeds ADC channel 9 (ADC_CH9); implemented as ADCAdvanced::differentialViaOPA()/differentialViaOPA1() (invert CH1 => Vout = Vch0 - Vch1). | Compile + hardware differential measurement. |
