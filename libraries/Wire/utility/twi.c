...
{
  // Preserve TWGCE while changing the 7-bit slave address.
  address &= 0x7Fu;
  TWAR = (uint8_t)((address << 1) | (TWAR & 0x01));
}

void twi_setAddressMask(uint8_t mask)
{
#if defined(TWAMR)
  // TWAMR[7:1] masks corresponding address bits; bit 0 is reserved.
  TWAMR = (uint8_t)((mask & 0x7Fu) << 1);
#else
  (void)mask;
#endif
}

void twi_setGeneralCall(bool enable)
{
  if (enable) TWAR |= 0x01;
  else TWAR &= (uint8_t)~0x01;
}
...
{
  // Avoid divide-by-zero and underflow for impossible rates.  For requests
  // above F_CPU/16 use the fastest representable TWI setting.
  if (frequency == 0) return;
  if (frequency >= (F_CPU / 16UL)) {
    TWBR = 0;
    return;
  }
  uint32_t br = ((F_CPU / frequency) - 16UL) / 2UL;
  if (br > 255UL) br = 255UL;
  TWBR = (uint8_t)br;
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  if (address > 0x7Fu) return 0;

  // A zero-byte request is valid at the Arduino API boundary but there is no
  // TWI transaction to perform.  Early return also prevents length-1 from
  // underflowing to 255.
...
ISR(TWI_vect)
{
  switch(TW_STATUS){
    ...
    case TW_MR_DATA_ACK: // data received, ack sent
      // Defensive capacity check: malformed state must never overwrite SRAM.
    ...
    case TW_MR_DATA_NACK: // data received, nack sent
      // put final byte into buffer, unless a corrupted state would exceed it
      if (twi_masterBufferIndex < TWI_BUFFER_LENGTH) {
        twi_masterBuffer[twi_masterBufferIndex++] = TWDR;
      } else {
        twi_error = 0xFE;
    ...
    case TW_ST_DATA_ACK: // byte sent, ack returned
      // Defensive state guard: never index beyond the slave TX buffer even if
      // an unexpected ISR state is observed.
      if (twi_txBufferIndex >= twi_txBufferLength || twi_txBufferIndex >= TWI_BUFFER_LENGTH) {
        TWDR = 0x00;
        twi_reply(0);
        break;
      }
