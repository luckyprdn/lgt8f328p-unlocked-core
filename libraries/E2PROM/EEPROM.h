...
//
// Recovery-safe profile: runtime partition-size changes are clamped to 1KB
// logical EEPROM (1020 user bytes / 2KB physical Flash). This deliberately
// prevents change_size()/lgt_eeprom_init() from expanding into bootloader or
// application Flash until a hardware recovery path is available.
//
...
//
	...
	#define	lgt_eeprom_SWM_ON()   do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr | 0x10u); SREG = _lgt_s; } while(0)
	#define	lgt_eeprom_SWM_OFF()  do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr & 0xEFu); SREG = _lgt_s; } while(0)
	#define	lgt_eeprom_reset()    do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr | 0x20u); SREG = _lgt_s; } while(0)
#else
...
// ------------------ OLD LGT EEPROM API ---------------------------
	...
	#if defined( __LGT8FX8P__ ) || defined( __LGT_EEPROM_LIB_FOR_328D__ )
		#define	e2pReset()	do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr | 0x20u); SREG = _lgt_s; } while(0)
		#define	e2pSWMON()	do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr | 0x10u); SREG = _lgt_s; } while(0)
		#define	e2pSWMOFF()	do { uint8_t _lgt_s = SREG; uint8_t _lgt_eccr = (uint8_t)(ECCR & 0x7Fu); cli(); ECCR = 0x80; ECCR = (uint8_t)(_lgt_eccr & 0xEFu); SREG = _lgt_s; } while(0)
	#else
		#define	e2pReset()
		#define	e2pSWMON()
		#define	e2pSWMOFF()
		...
		class EEPROMClass
		{
		  public:
			...
			{
				lgt_eeprom_init( number_of_1KB_pages );
