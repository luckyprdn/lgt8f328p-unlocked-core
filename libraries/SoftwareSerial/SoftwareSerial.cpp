...
{
  if (tx >= NUM_PIN_TABLE_ENTRIES || digitalPinToPort(tx) == NOT_A_PIN) {
    _transmitBitMask = 0; _transmitPortRegister = NULL; return;
  }
...
{
  _receivePin = rx;
  if (rx >= NUM_PIN_TABLE_ENTRIES || digitalPinToPort(rx) == NOT_A_PIN) {
    _receiveBitMask = 0; _receivePortRegister = NULL; _pcint_maskreg = NULL; return;
  }
  pinMode(rx, INPUT);
  if (!_inverse_logic)
...
void SoftwareSerial::begin(long speed)
{
  ...
  if (speed <= 0 || _transmitPortRegister == NULL) return;
