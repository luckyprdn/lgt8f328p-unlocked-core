...
#define ARDUINO_MAIN
#define LGT_DISABLE_DIGITAL_FASTPATH
#include "wiring_private.h"
#include "pins_arduino.h"

void pinMode(uint8_t pin, uint8_t mode)
{
	if (pin >= NUM_PIN_TABLE_ENTRIES) return;
	...
	if((mode == ANALOG) && (pin == DAC0)) { // enable DAC
		...
		// Recovery-safe build: E0/E2 remain owned by SWD/SWC. Do not disable
		// the only convenient ISP/debug recovery path just because pinMode()
		// was called. GPIO access to these pins therefore remains unavailable.
		return;
#else
...
{
	if (pin >= NUM_PIN_TABLE_ENTRIES) return LOW;
	uint8_t timer = digitalPinToTimer(pin);
	uint8_t bit = digitalPinToBitMask(pin);
	uint8_t port = digitalPinToPort(pin);
