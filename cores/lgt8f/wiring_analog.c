...
static int __analogRead(uint8_t pin, uint8_t compensate)
{
	uint16_t pVal;
	...
	#if defined(__LGT8FX8P__)
	if (compensate) pVal = (pVal + nVal) >> 1;
	#endif
#else
...
#if defined(__LGT8F__)
	if(analog_resbit == 0)
		return __analogRead(pin, 1);

	if(analog_resdir == 1) {
		return __analogRead(pin, 1) >> analog_resbit;
	} else {
		return __analogRead(pin, 1) << analog_resbit;
	}
#else
...
{
#if defined(__LGT8F__)
  int value = __analogRead(pin, 0);
  if (analog_resbit == 0) return value;
  return analog_resdir ? (value >> analog_resbit) : (value << analog_resbit);
#else
